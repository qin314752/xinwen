...
@section('content')
<div class="table-header">公益管理</div>
<div>
  <div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline no-footer">
    ...
    <div class="row">
      <form id="myform" action="/admin/gongyi/index" method="get">
        <div class="col-xs-6">
      ...
      <form  action="/admin/gongyi/index" method="get">
    ...
    <table  class="table table-striped table-bordered table-hover dataTable no-footer DTTT_selectable" role="grid" aria-describedby="dynamic-table_info">
      <thead>
        <tr role="row" style="width: 292px;">
          ...
          <th class="hidden-480 sorting" style="width:206px" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" aria-label="Clicks: activate to sort column ascending">公益组织</th>
      ...
      @foreach ($arr as $v)
        <tr role="row" class="odd even tr">
            ...
            <td><div  style="margin-top: 10px">{{$v->name}}</div></td>
	          <td><div  style="margin-top: 10px">{{$v->zuzhi}}</div></td>
            ...
            @endif
	         ...
	         <td class="center">
		         <div class="hidden-sm hidden-xs action-buttons">
					<a class="blue c" href="/admin/gongyi/view?id={{$v->id}}&name={{$v->name}}&content={{$v->content}}&pic={{$v->pic}}&time={{$v->time}}"><i class="ace-icon fa fa-search-plus bigger-130"></i></a>
					<a class="green b" href="/admin/gongyi/update?id={{$v->id}}&name={{$v->name}}&content={{$v->content}}&pic={{$v->pic}}&time={{$v->time}}"><i class="ace-icon fa fa-pencil bigger-130"></i></a>
...
$('.a').click(function(){
	...
	$.get("{{url('/admin/gongyi/del')}}",{id:id},function(data){
		if(data == 1){
