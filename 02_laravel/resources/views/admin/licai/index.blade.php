@extends('/lang/admin')
@section('title', '理财产品管理')

@section('content')
<div class="table-header">理财产品管理</div>
<div>
  <div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline no-footer">
    ...
    <div class="row">
      <form id="myform" action="/admin/licai/index" method="get">
        <div class="col-xs-6">
      ...
      <form  action="/admin/licai/index" method="get">
            ...
            @endif
	         ...
	         <td class="center">
		         <div class="hidden-sm hidden-xs action-buttons">
					<a class="blue c" href="/admin/licai/view?id={{$v->id}}&name={{$v->name}}&content={{$v->content}}&pic={{$v->pic}}&time={{$v->time}}"><i class="ace-icon fa fa-search-plus bigger-130"></i></a>
					<a class="green b" href="/admin/licai/update?id={{$v->id}}&name={{$v->name}}&content={{$v->content}}&pic={{$v->pic}}&time={{$v->time}}"><i class="ace-icon fa fa-pencil bigger-130"></i></a>
...
$('.a').click(function(){
	...
	$.get("{{url('/admin/licai/del')}}",{id:id},function(data){
		if(data == 1){
